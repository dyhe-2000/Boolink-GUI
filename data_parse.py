...
INIT_COND = int(np.loadtxt('Initialization Setting.txt'))   #2500
TIME_STEP = int(np.loadtxt('Time Steps Setting.txt'))       #20+1   # {0,1,2...21}, for example. 0 = initial. 1,2,3.. = updates
TIME_STEP += 1

'''
NODE_NUM = int(sum(1 for line in open('Node Name and their initial state.txt'))) # 81
#NODE_NUM = int(sum(1 for line in open('Node Name and their initial state.txt')) + 1) # 82
'''
NODE_NUM = 0
for line in open('Node Name and their initial state.txt'):
    if(len(line.strip())):
        NODE_NUM += 1
...
for line in in_file:
    ...
    if(line.rstrip() == ic_line):
        ...
        # skip the next 3 lines (including the 'fake' input)
    ...
    # else: is the line the beginning of the results after one simulation time step?
        ...
        # skip one line, because of the 'fake' input
        # IMP: the top most line is popped each time in_file is iterated
        skip = 0
        for line_skip in in_file:
            # do nothing with line_skip
